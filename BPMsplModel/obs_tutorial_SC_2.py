...
class SteadyObserverLocation():
    ...
    def steady_observer_model(self, num_nodes = 1):
        # num_nodes = observer_data['num_nodes']
        aircraft_location = self.observer_data['aircraft_position']
        x = self.observer_data['x']
        y = self.observer_data['y']
        z = self.observer_data['z']
        
        time_vectors = self.observer_data['time']
        num_obs = self.observer_data['num_observers']
        ...
        thrust_dir = csdl.expand(thrust_dir, exp_shape, action='i->aib')
        ...
        self.SteadyObserver = {'rel_obs_x_pos': rel_obs_x_pos,
