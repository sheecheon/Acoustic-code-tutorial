...
def obs_time_computation(obs_t0, Edgewise_input, alpha, tau):   # num_azim, num_radial, num_blades / (20, 40, 2)
    ...
    tau = Edgewise_input['tau']
    
    num_radial = radial.size
    num_azim = azimuth.size
    ...
    obs_t = csdl.ImplicitVariable(name='obs_t', value=obs_t0)     #0.04483238 
    
    # Expansion
    target_shape = (num_azim, num_radial, num_blades)
    exp_x = csdl.expand(obs_x, target_shape)
    exp_y = csdl.expand(obs_y, target_shape)
    exp_z = csdl.expand(obs_z, target_shape)
    
    exp_r = csdl.expand(csdl.reshape(radial, (num_radial,)), target_shape, 'i->aib')
    exp_azim = csdl.expand(azimuth, target_shape, 'ij->iaj')
    exp_tau = csdl.expand(tau, target_shape, 'i->iab')
                       
    exp_obs_t = csdl.expand(obs_t, csdl.reshape(obs_t, (num_radial, )), 'i->aib')
    
    # obs_t = csdl.expand(0.04483238, target_shape)
    residual_1 = exp_obs_t - ((((exp_x - exp_r*csdl.cos(exp_azim)*csdl.cos(alpha)) + U*(exp_obs_t - exp_tau))**2 + (exp_y - exp_r*csdl.sin(exp_azim)*csdl.cos(alpha))**2 + (exp_z - exp_r*csdl.cos(exp_azim)*csdl.sin(alpha))**2)**(0.5))/c0 - exp_tau
    residual_2 = 1 - (1/c0)*(((exp_x - exp_r*csdl.cos(exp_azim)*csdl.cos(alpha)) + U*(exp_obs_t - exp_tau))**2 + (exp_y - exp_r*csdl.sin(exp_azim)*csdl.cos(alpha))**2 + (exp_z - exp_r*csdl.cos(exp_azim)*csdl.sin(alpha))**2)**(-0.5)*(exp_x - exp_r*csdl.cos(exp_azim)*csdl.cos(alpha) + U*(exp_obs_t - exp_tau))
...
num_observers = 1
num_freq = len(freq)   
num_blades = int(Edgewise_input['B'])
...
obs_t0 = 30*Psi/(np.pi*rpm)   # obs_t0 = 30*Psi/(np.pi*rpm)   # initial guess for Newton method
# obs_time = obs_time_computation(obs_t0, Edgewise_input, alpha, tau)   # shape = (num_azim, num_radial , num_blades)

# Due to limited memory, observer time is imported from HJ's code
obs_time = io.loadmat('OBStime_SUI.mat')
# obs_time = io.loadmat('OBStime_Burley.mat')
# obs_time = io.loadmat('OBStime_BO105Case1.mat')
obs_time = obs_time['ObsTime']     # shape = (num_azim, num_radial , num_blades)
...
totalSPL0 = 10*csdl.log(BPMsum, 10)  #eq. 20 / SPL13 in ref. HJ's code

# outband transformation
outband = Edgewise_input['outBand']
...
## Check location where x<=0, when log(x)!!
re_shape = num_nodes*num_observers*num_radial*num_azim*num_blades
check_splp = csdl.reshape(splp, (re_shape, num_freq)).value
check_spls = csdl.reshape(spls, (re_shape, num_freq)).value
check_spla = csdl.reshape(spla, (re_shape, num_freq)).value
spl_BLUNT = csdl.reshape(spl_BLUNT, (re_shape, num_freq)).value
check_BPMsum = csdl.reshape(BPMsum, (re_shape, num_freq)).value
...
check_totalSPL = csdl.reshape(totalSPL, (re_shape, num_freq)).value
check_noise1 = csdl.reshape(noise_con1, (re_shape, num_freq)).value
check_noise2 = csdl.reshape(noise_con2, (re_shape, num_freq)).value

# ========== Try3 : Following HJ's code (0.082s) for verification ==============
start0 = time.time()
...
A = final_SPL0.value
check_tIndx = ((time_indx1 + time_indx2)/2)[:, :, 0]

# # ============ Try1 & Try2 : reshape for noise contribution allocation ==============
# re_shape = num_nodes*num_observers*num_radial*num_azim*num_blades
