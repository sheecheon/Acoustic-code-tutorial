...
class BPMsplModel():
    ...
    def TBLTE(self):
        ...
        f = np.array([400,500,630,800,1000,1250,1600,2000,2500,3150,4000,5000,6300,
                          8000,10000,12500,16000,20000,25000,31500,40000,50000]) # 1/3 octave band 
        # f = np.array([16000])
        ...
        f11 = (-83.607*b0) + 8.138
        f12 = (-817.81*(b0**3)) + (355.21*(b0**2)) - (135.024*b0) + 10.619
    ...
    def TE_BLUNT(self):
        ...
        #========================== Variable expansion =========================
        TE_thick = csdl.expand(h, target_shape, 'ij->aijcde') # symbol 'h': temp. value
        slope_angle = csdl.expand(Psi, target_shape, 'ij->aijcde') # symbol 'Psi': temp.value
