...
D = SteadyObserverLocationTutorial(observer_data, velocity_data)

rel_obs = D.steady_observer_model()
# rel_obs_dist = rel_obs[0].value
print(rel_obs['rel_obs_dist'].value)
...
# rel_obs_dist = 2.26999959 # This will be replaced by internal code

def gl_spl(observer_data, input_data, freq_band, rel_obs, num_nodes=1):
    ...
    S = rel_obs['rel_obs_dist']        #rel_obs_dist = temporary
...
recorder.start()
spl = gl_spl(observer_data, input_data, freq_band, rel_obs)
print(spl.value)
recorder.stop()
        
    # recorder = csdl.Recorder(inline=True)
    # recorder.start()
    # rel_obs_dist = steady_observer_model(observer_data, velocity_data)
    # recorder.stop()
